...
/* Fiar una venta existente (Live). Anti doble-fiado por sale_id.
 * amount = deuda FINAL; discount = rebaja aplicada (migración 11). */
export async function rpcFiarVenta({ saleId, amount, discount }) {
  const monto = sanitizeNumber(amount, { min: 0.01 });
  if (monto === null) return { data: null, error: new Error('Monto inválido') };
  const rebaja = sanitizeNumber(discount ?? 0, { min: 0 }) ?? 0;
  const supabase = createClient();
...
/* Cobrar un apartado de Live (update + payment en una transacción).
 * amount = monto FINAL cobrado; discount = rebaja aplicada (migración 11). */
export async function rpcCobrarVenta({ saleId, paymentId, amount, method, discount }) {
  const monto = sanitizeNumber(amount, { min: 0.01 });
  if (monto === null) return { data: null, error: new Error('Monto inválido') };
  const rebaja = sanitizeNumber(discount ?? 0, { min: 0 }) ?? 0;
  const supabase = createClient();
