...
export async function rpcAplicarAbono({ paymentId, clientId, amount, method }) {
...
export async function rpcFiarVenta({ saleId, amount }) {
  const monto = sanitizeNumber(amount, { min: 0.01 });
...
export async function rpcCobrarVenta({ saleId, paymentId, amount, method }) {
  const monto = sanitizeNumber(amount, { min: 0.01 });
  if (monto === null) return { data: null, error: new Error('Monto inválido') };
  const supabase = createClient();
