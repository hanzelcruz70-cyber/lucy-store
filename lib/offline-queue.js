...
/* ============================================================
 * Cola offline de Mi Prenda (outbox pattern)
...
const LS_QUEUE = 'miprenda_outbox';
const LS_EMITS = 'miprenda_emit'; // id de la última emisión de eventos
const LEGACY_QUEUE = 'pacapos_outbox'; // clave pre-renombre (2026-09)

/* Migración: mueve la cola vieja de PacaPOS a la clave nueva (una vez) */
function migrateLegacy() {
  try {
    const old = localStorage.getItem(LEGACY_QUEUE);
    if (old) {
      const current = localStorage.getItem(LS_QUEUE);
      const parsed = JSON.parse(old);
      if (!current && Array.isArray(parsed) && parsed.length > 0) {
        localStorage.setItem(LS_QUEUE, old); // conserva operaciones pendientes
      }
...
export function enqueueOp(op) {
  ...
  const entry = {
    ...
    ts: Date.now(),
    tries: 0,
    // Fecha ORIGINAL del negocio (no la del sync futuro): la usan los
    // procesadores para created_at en la BD
