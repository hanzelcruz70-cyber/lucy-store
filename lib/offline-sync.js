...
const processors = {
  ...
  async fiado_directo(supabase, ctx, op) {
    const p = op.payload;
    // Idempotente: si ya existe la venta con el id local, listo
    const { data: existingSale } = await supabase.from('sales').select('id').eq('id', p.saleLocalId || p.localId).limit(1);
    if (existingSale && existingSale.length > 0) return { ok: true };

    // 1) Venta fiada: genera folio y aparece en Movimientos del día
    const { data: sale, error: errSale } = await supabase
      .from('sales')
      .insert({
        id: p.saleLocalId || p.localId,
        total: p.amount,
        items_count: 1,
        channel: 'mostrador',
        payment_method: 'fiado',
      ...
      .select('id')
      .single();
    if (errSale && errSale.code !== '23505') return { ok: false, error: errSale };

    // 2) Deuda ligada a esa venta (idempotente por id local)
    const { data: existingDebt } = await supabase.from('debts').select('id').eq('id', p.localId).limit(1);
    if (!existingDebt || existingDebt.length === 0) {
      const { error: errDebt } = await supabase.from('debts').insert({
        id: p.localId,
        ...
        status: 'pendiente',
        sale_id: (sale && sale.id) || p.saleLocalId || p.localId,
        store_id: ctx.storeId,
        user_id: ctx.userId,
      });
