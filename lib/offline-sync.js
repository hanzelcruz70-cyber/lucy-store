...
const processors = {
  ...
  async sale(supabase, ctx, op) {
    ...
    // p_discount (migración 11): p.total ya es el monto FINAL; la rebaja
    // viaja aparte y queda en sales.discount.
    const { data, error } = await supabase.rpc('registrar_venta', {
      ...
      p_discount: Math.max(0, Number(p.discount) || 0),
    });
    if (error) return { ok: false, error };
    return { ok: true, data };
  },
  ...
  async cash_cut(supabase, ctx, op) {
    ...
    // Ganancia del día (puede ser NEGATIVA en un día malo — sanitizeNumber
    // la cortaría en 0; por eso aquí va cruda con check de número finito)
    const profitRaw = Number(p.profitTotal);
    const profit = Number.isFinite(profitRaw) ? profitRaw : null;
