 ...
 * efectivo + cobros de apartados en efectivo − gastos + fondo inicial). */
function addCajaNeta(ws, row, { esperado, fisico, fondo, abonos, cobrosApartados, gastosEfectivo, transferencias }) {
  const rows = [
    ['Fondo inicial (caja con la que abriste)', fondo, 'neutral'],
    ['Ventas en efectivo de hoy', esperado - fondo - abonos - cobrosApartados + gastosEfectivo, 'neutral'],
    ['Abonos en efectivo recibidos', abonos, 'neutral'],
    ['Apartados de días anteriores cobrados en efectivo', cobrosApartados, 'neutral'],
  ...
  payments.forEach((p, idx) => {
    ...
    addDataRow(
      ...
      [
        ...
        p.sale_id ? 'Apartado de Live cobrado' : 'Abono de deuda',
...
export async function buildReportWorkbook({
  ...
  /* ===== ARQUEO (desde fila 9): desglose que cuadra con el cierre ===== */
  const arqueo = {
    esperado,
    fisico,
    fondo,
    abonos: abonosEfectivo,
    cobrosApartados: cobrosApartadosEfectivo,
    gastosEfectivo: gastos,
    transferencias,
  };
  let r = addCajaNeta(ws, 9, arqueo);
